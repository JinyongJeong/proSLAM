...
StereoTracker::StereoTracker(StereoTrackerParameters* parameters_) : BaseTracker(parameters_), _parameters(parameters_)
{
...
// ds creates a new Frame for the given images, retrieves the correspondences relative to the previous Frame, optimizes
// the current frame pose and updates landmarks
...
void StereoTracker::_recoverPoints(Frame* current_frame_)
{
  ...
  for (FramePoint* point_previous : _lost_points)
  {
    ...
    const cv::Rect_<float> region_of_interest_left(corner_left.x, corner_left.y, regional_full_height,
    ...
    if (cv::norm(point_previous->descriptorLeft(), descriptor_left, SRRG_PROSLAM_DESCRIPTOR_NORM) >
    ...
    const cv::Rect_<float> region_of_interest_right(corner_right.x, corner_right.y, regional_full_height,
                                                    regional_full_height);
    ...
    // ds if descriptor distance is to high
    if (cv::norm(point_previous->descriptorRight(), descriptor_right, SRRG_PROSLAM_DESCRIPTOR_NORM) >
        maximum_descriptor_distance)
    {
      continue;
    }
    ...
    if (keypoint_buffer_left[0].pt.x - keypoint_buffer_right[0].pt.x <
        _stereo_framepoint_generator->parameters()->minimum_disparity_pixels)
    {
      continue;
    }

    // ds allocate a new point connected to the previous one
    FramePoint* current_point = current_frame_->createFramepoint(
        keypoint_buffer_left[0], descriptor_left, keypoint_buffer_right[0], descriptor_right,
  ...
  LOG_DEBUG(std::cerr << "StereoTracker::_recoverPoints|recovered points: " << _number_of_recovered_points << "/"
                      << _number_of_lost_points << std::endl)
}
}
