...
class BaseTracker
{
  ...
  // ds functionality
public:
  //! @brief creates a new Frame for the given images, retrieves the correspondences relative to the previous Frame,
  //! optimizes the current frame pose and updates landmarks
  ...
  // ds getters/setters
public:
  const uint64_t& numberOfRecursiveRegistrations() const
  {
    return _number_of_recursive_registrations;
  }
  void setCameraLeft(const Camera* camera_left_)
  {
    _camera_left = camera_left_;
    _has_odometry = false;
  }
  void setOdometry(const TransformMatrix3D& odometry_)
  {
    _odometry = odometry_;
    _has_odometry = true;
  }
  void setAligner(BaseFrameAligner* pose_optimizer_)
  {
    _pose_optimizer = pose_optimizer_;
  }
  void setFramePointGenerator(BaseFramePointGenerator* framepoint_generator_)
  {
    _framepoint_generator = framepoint_generator_;
  }
  void setWorldMap(WorldMap* context_)
  {
    _context = context_;
  }
  void setIntensityImageLeft(const cv::Mat& intensity_image_left_)
  {
    _intensity_image_left = intensity_image_left_;
  }
  BaseFrameAligner* aligner()
  {
    return _pose_optimizer;
  }
  void setMotionPreviousToCurrent(const TransformMatrix3D& motion_previous_to_current_)
  {
    _previous_to_current_camera = motion_previous_to_current_;
  }
  BaseFramePointGenerator* framepointGenerator()
  {
    return _framepoint_generator;
  }
  const BaseFramePointGenerator* framepointGenerator() const
  {
    return _framepoint_generator;
  }
  const Count totalNumberOfTrackedPoints() const
  {
    return _total_number_of_tracked_points;
  }
  const Count totalNumberOfLandmarks() const
  {
    return _total_number_of_landmarks;
  }
  const real meanTrackingRatio() const
  {
    return _mean_tracking_ratio;
  }
  const real meanNumberOfKeypoints() const
  {
    return _mean_number_of_keypoints;
  }
  const real meanNumberOfFramepoints() const
  {
    return _mean_number_of_framepoints;
  }

  // ds helpers
protected:
  // ds retrieves framepoint correspondences between previous and current frame
  void _track(Frame* previous_frame_, Frame* current_frame_, const TransformMatrix3D& previous_to_current_,
  ...
  void _registerRecursive(Frame* frame_previous_, Frame* frame_current_, TransformMatrix3D previous_to_current_,
