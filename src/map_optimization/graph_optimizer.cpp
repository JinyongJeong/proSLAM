...
namespace proslam
{
GraphOptimizer::GraphOptimizer(GraphOptimizerParameters* parameters_)
  : _parameters(parameters_), _optimizer(nullptr), _vertex_frame_last_added(nullptr)
{
...
void GraphOptimizer::configure()
{
  ...
  if (_parameters->optimization_algorithm == "GAUSS_NEWTON" && _parameters->linear_solver_type == "CHOLMOD" &&
      !_parameters->enable_full_bundle_adjustment)
  {
    ALLOCATE_SOLVER(OptimizerGaussNewton, LinearSolverCholmod6x3, BlockSolver6x3)
  }
  else if (_parameters->optimization_algorithm == "GAUSS_NEWTON" && _parameters->linear_solver_type == "CSPARSE" &&
           !_parameters->enable_full_bundle_adjustment)
  {
    ALLOCATE_SOLVER(OptimizerGaussNewton, LinearSolverCSparse6x3, BlockSolver6x3)
  }

  else if (_parameters->optimization_algorithm == "GAUSS_NEWTON" && _parameters->linear_solver_type == "CHOLMOD" &&
           _parameters->enable_full_bundle_adjustment)
  {
  ...
  else if (_parameters->optimization_algorithm == "LEVENBERG" && _parameters->linear_solver_type == "CHOLMOD" &&
           !_parameters->enable_full_bundle_adjustment)
  {
    ALLOCATE_SOLVER(OptimizerLevenberg, LinearSolverCholmod6x3, BlockSolver6x3)
  }
  else if (_parameters->optimization_algorithm == "LEVENBERG" && _parameters->linear_solver_type == "CSPARSE" &&
           !_parameters->enable_full_bundle_adjustment)
  {
  ...
  LOG_INFO(std::cerr << "GraphOptimizer::configure|allocated optimization algorithm: "
                     << _parameters->optimization_algorithm << " with solver: " << _parameters->linear_solver_type
                     << std::endl)
...
void GraphOptimizer::writePoseGraphToFile(const WorldMap* world_map_, const std::string& file_name_) const
{
  ...
  LOG_INFO(std::cerr << "GraphOptimizer::writePoseGraphToFile|saving final pose graph to file: " << file_name_
                     << std::endl)

  // ds get a graph handle
  g2o::OptimizableGraph* pose_graph = new g2o::OptimizableGraph();
  g2o::VertexSE3* vertex_frame_last_added = 0;
  std::map<Landmark*, g2o::VertexPointXYZ*, std::less<Landmark*>,
           Eigen::aligned_allocator<std::pair<Landmark*, g2o::VertexPointXYZ*>>>
      landmarks_in_pose_graph;
  ...
  for (const FramePointerMapElement& frame : world_map_->frames())
  {
    ...
    else
    {
      ...
      _setPoseEdge(pose_graph, vertex_frame_current, vertex_frame_last_added,
                   frame.second->previous()->worldToRobot() * frame.second->robotToWorld(),
                   _parameters->base_information_frame, _parameters->free_translation_for_poses);
    }
    ...
    for (FramePoint* framepoint : frame.second->points())
    {
      ...
      if (landmark)
      {
        ...
        _setPointEdge(pose_graph, vertex_frame_current, vertex_landmark, framepoint->robotCoordinates(),
                      1 / framepoint->depthMeters());
      }
    }
  ...
  for (const FramePointerMapElement& frame : world_map_->frames())
  {
    ...
    if (frame.second->localMap() && frame.second->localMap()->closures().size() > 0)
    {
      ...
      for (const LocalMap::ClosureConstraint& closure : frame.second->localMap()->closures())
      {
        ...
        _setPoseEdge(pose_graph, pose_graph->vertex(frame.second->localMap()->keyframe()->identifier()),
                     pose_graph->vertex(closure.local_map->keyframe()->identifier()), closure.relation,
                     _parameters->base_information_frame, _parameters->free_translation_for_poses,
...
void GraphOptimizer::addFrame(Frame* frame_)
{
  ...
  else
  {
    ...
    // ds we can connect it to the preceeding frame by adding the odometry measurement
    _setPoseEdge(_optimizer, vertex_frame_current, _vertex_frame_last_added,
                 frame_->previous()->worldToRobot() * frame_->robotToWorld(), information_factor,
                 _parameters->free_translation_for_poses, _parameters->enable_robust_kernel_for_poses);
  }
  ...
  if (local_map)
  {
    ...
    if (_local_maps_in_graph.find(local_map->identifier()) == _local_maps_in_graph.end())
    {
      ...
      for (const LocalMap::ClosureConstraint& closure : local_map->closures())
      {
        ...
        // ds retrieve closure edge
        _setPoseEdge(_optimizer, _optimizer->vertex(local_map->keyframe()->identifier()),
                     _optimizer->vertex(reference_frame->identifier()), closure.relation, information_factor,
                     _parameters->free_translation_for_poses, _parameters->enable_robust_kernel_for_poses);
      }
    }
  }
...
void GraphOptimizer::addFrameWithLandmarks(Frame* frame_)
{
  ...
  if (!_vertex_frame_last_added)
  {
    // ds add landmark measurements contained in the frame by scanning its framepoints - porting weights from previous
    // optimization
    for (FramePoint* framepoint : frame_->points())
    {
      ...
      if (landmark)
      {
        ...
        _setPointEdge(_optimizer, vertex_frame_current, vertex_landmark, framepoint->robotCoordinates(),
                      landmark->numberOfUpdates() / framepoint->depthMeters());
      }
    }
  ...
  else
  {
    ...
    // ds we can connect it to the preceeding frame by adding the odometry measurement
    _setPoseEdge(_optimizer, vertex_frame_current, _vertex_frame_last_added,
                 frame_->previous()->worldToRobot() * frame_->robotToWorld(), _parameters->base_information_frame,
...
void GraphOptimizer::optimizeFrames(WorldMap* world_map_)
{
  ...
  // ds directly backpropagate solution to frames - without updating the local maps (we want to keep the fine-grained,
  // frame-wise g2o estimate)
...
void GraphOptimizer::_setPoseEdge(g2o::OptimizableGraph* optimizer_, g2o::OptimizableGraph::Vertex* vertex_from_,
                                  g2o::OptimizableGraph::Vertex* vertex_to_,
                                  const TransformMatrix3D& transform_from_to_, const real& information_factor_,
                                  const bool& free_translation_, const bool& enable_robust_kernel_) const
{
  ...
  if (enable_robust_kernel_)
  {
    edge_pose->setRobustKernel(new g2o::RobustKernelCauchy());
  }
  optimizer_->addEdge(edge_pose);
}

void GraphOptimizer::_setPointEdge(g2o::OptimizableGraph* optimizer_, g2o::VertexSE3* vertex_frame_,
                                   ...
                                   const real& information_factor_) const
{
  ...
  if (_parameters->enable_robust_kernel_for_landmarks)
  {
    landmark_edge->setRobustKernel(new g2o::RobustKernelCauchy());
  }
