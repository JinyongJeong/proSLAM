...
void Relocalizer::detectClosures(LocalMap* local_map_query_)
{
  ...
  // _place_database에 입력 local map 데이터를 추가 (최소 plane database 사이즈가 안되었을떄)
  ...
  {
    // ds matching result container: a map that contains a vector of matches to the current image for each reference
    // image
    ...
    _place_database.matchAndAdd(local_map_query_->appearances(), matches_per_reference_image,
                                _parameters->maximum_descriptor_distance);
    local_map_query_->appearances().clear();
    ...
    for (Count index_reference_local_map = 0; index_reference_local_map < maximum_index_reference;
         ++index_reference_local_map)
    {
      ...
      const real relative_number_of_matches =
          static_cast<real>(multiple_matches_mixed.size()) / number_of_query_matchables;
      ...
      for (const HBSTTree::Match& match : multiple_matches_mixed)
      {
        ...
        try
        {
          ...
          multiple_matches_per_landmark.at(query_landmark_identifier)
              .push_back(Closure::Candidate(landmark_query, landmark_reference, match.distance));
        }
        catch (const std::out_of_range& /*exception*/)
        {
          // ds initialize the first match for the given query point
          multiple_matches_per_landmark.insert(std::make_pair(
              query_landmark_identifier,
      ...
      _closures.push_back(new Closure(local_map_query_, _added_local_maps[index_reference_local_map],
                                      multiple_matches_per_landmark.size(), relative_number_of_matches,
                                      correspondences));
    }
  }
...
Closure::Correspondence* Relocalizer::_getCorrespondenceNN(const Closure::CandidateVector& matches_)
{
  ...
  if (match_best && count_best > _parameters->minimum_matches_per_correspondence)
  {
    ...
    return new Closure::Correspondence(match_best->query, match_best->reference, count_best,
                                       static_cast<real>(count_best) / matches_.size());
  }
