...
void UVDAligner::initialize(const Frame* frame_previous_, const Frame* frame_current_,
...
void UVDAligner::linearize(const bool& ignore_outliers_)
{
  ...
  for (Index index_point = 0; index_point < _frame_current->points().size(); index_point++)
  {
    ...
    jacobian_projection << inverse_predicted_d, 0, -predicted_uvd_in_camera.x() * inverse_predicted_d_squared, 0,
        inverse_predicted_d, -predicted_uvd_in_camera.y() * inverse_predicted_d_squared, 0, 0, 1;
