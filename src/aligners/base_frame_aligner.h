...
public:
  // ds pure virtual
  virtual void initialize(const Frame* frame_previous_, const Frame* frame_current_,
                          const TransformMatrix3D& previous_to_current_) = 0;

  // ds setters/getters
public:
  void setMaximumDepthNearMeters(const real& maximum_depth_near_meters_)
  {
    _maximum_depth_near_meters = maximum_depth_near_meters_;
  }
  void setMaximumDepthFarMeters(const real& maximum_depth_far_meters_)
  {
    _maximum_depth_far_meters = maximum_depth_far_meters_;
  }
  inline const TransformMatrix3D& previousToCurrent() const
  {
    return _previous_to_current;
  }

  // ds dynamic weighting
  void setEnableWeightsTranslation(const bool enable_weights_translation_)
  {
    _enable_weights_translation = enable_weights_translation_;
  }
