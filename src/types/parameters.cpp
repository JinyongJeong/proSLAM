...
std::string ParameterCollection::banner =
    ...
    "-depth-mode (-dm):                       depth tracking (-topic-image-left: intensity image, -topic-image-right: "
    "depth)\n"
    ...
    "-disable-bundle-adjustment (-dba):       disables periodic bundle adjustment for landmarks and "
    "frames\n" DOUBLE_BAR;
...
#define PARSE_PARAMETER(YAML_NODE, YAML_GROUP_NAME, STRUCT_NAME, PARAMETER_NAME, PARAMETER_TYPE)                       \
  ...
  {                                                                                                                    \
    ...
    /*std::cerr << "parameter name: '" << #STRUCT_NAME << ": " << #PARAMETER_NAME << "' value: '" <<                   \
     * STRUCT_NAME->PARAMETER_NAME << "'" << std::endl;*/                                                              \
  }                                                                                                                    \
  catch (const YAML::TypedBadConversion<PARAMETER_TYPE>& exception_)                                                   \
  {                                                                                                                    \
    LOG_WARNING(std::cerr << "unable to parse parameter: '" << #STRUCT_NAME << ": " << #PARAMETER_NAME                 \
                          << "' (exception: '" << exception_.what() << "')" << std::endl)                              \
  }
...
{
  std::cerr << "LandmarkParameters::print|minimum_number_of_forced_updates: " << minimum_number_of_forced_updates
            << std::endl;
}
...
{
  std::cerr << "WorldMapParameters::print|minimum_distance_traveled_for_local_map: "
            << minimum_distance_traveled_for_local_map << std::endl;
  std::cerr << "WorldMapParameters::print|minimum_degrees_rotated_for_local_map: "
            << minimum_degrees_rotated_for_local_map << std::endl;
  std::cerr << "WorldMapParameters::print|minimum_number_of_frames_for_local_map: "
            << minimum_number_of_frames_for_local_map << std::endl;
  landmark->print();
  local_map->print();
}
...
{
  std::cerr << "BaseFramepointGeneratorParameters::print|target_number_of_keypoints_tolerance: "
            << target_number_of_keypoints_tolerance << std::endl;
  std::cerr << "BaseFramepointGeneratorParameters::print|detector_threshold_minimum: " << detector_threshold_minimum
            << std::endl;
  std::cerr << "BaseFramepointGeneratorParameters::print|detector_threshold_maximum_change: "
            << detector_threshold_maximum_change << std::endl;
  std::cerr << "BaseFramepointGeneratorParameters::print|matching_distance_tracking_threshold: "
            << matching_distance_tracking_threshold << std::endl;
  std::cerr << "BaseFramepointGeneratorParameters::print|enable_keypoint_binning: " << enable_keypoint_binning
            << std::endl;
...
{
  std::cerr << "StereoFramepointGeneratorParameters::print|maximum_matching_distance_triangulation: "
            << maximum_matching_distance_triangulation << std::endl;
  std::cerr << "StereoFramepointGeneratorParameters::print|minimum_disparity_pixels: " << minimum_disparity_pixels
            << std::endl;
...
BaseTrackerParameters::BaseTrackerParameters(const LoggingLevel& logging_level_)
  : Parameters(logging_level_), aligner(new AlignerParameters(logging_level_))
{
...
{
  std::cerr << "BaseTrackerParameters::print|minimum_number_of_landmarks_to_track: "
            << minimum_number_of_landmarks_to_track << std::endl;
  std::cerr << "BaseTrackerParameters::print|maximum_number_of_landmark_recoveries: "
            << maximum_number_of_landmark_recoveries << std::endl;
  aligner->print();
}
...
{
  std::cerr << "RelocalizerParameters::print|preliminary_minimum_interspace_queries: "
            << preliminary_minimum_interspace_queries << std::endl;
  std::cerr << "RelocalizerParameters::print|preliminary_minimum_matching_ratio: " << preliminary_minimum_matching_ratio
            << std::endl;
  std::cerr << "RelocalizerParameters::print|minimum_number_of_matches_per_landmark: "
            << minimum_number_of_matched_landmarks << std::endl;
  std::cerr << "RelocalizerParameters::print|minimum_matches_per_correspondence: " << minimum_matches_per_correspondence
            << std::endl;
  aligner->print();
}
...
void GraphOptimizerParameters::print() const
{
  ...
  std::cerr << "GraphOptimizerParameters::print|number_of_frames_per_bundle_adjustment: "
            << number_of_frames_per_bundle_adjustment << std::endl;
  std::cerr << "GraphOptimizerParameters::print|base_information_frame: " << base_information_frame << std::endl;
  std::cerr << "GraphOptimizerParameters::print|enable_robust_kernel_for_landmark_measurements: "
            << enable_robust_kernel_for_landmarks << std::endl;
}
...
ParameterCollection::ParameterCollection(const LoggingLevel& logging_level_)
  : Parameters(logging_level_), _parameters(this), number_of_parameters_detected(0), number_of_parameters_parsed(0)
{
...
void ParameterCollection::parseFromCommandLine(const int32_t& argc_, char** argv_)
{
  ...
  {
    if (!std::strcmp(argv_[number_of_checked_parameters], "-configuration") ||
        !std::strcmp(argv_[number_of_checked_parameters], "-c"))
    {
      number_of_checked_parameters++;
      if (number_of_checked_parameters == argc_)
      {
        break;
      }
  ...
  {
    LOG_WARNING(std::cerr << "ParameterCollection::parseParametersFromCommandLine|no configuration file specified "
                             "(running with internal settings)"
                          << std::endl)
  }
  else
  {
    ...
    {
      LOG_ERROR(std::cerr << "ParameterCollection::parseParametersFromCommandLine|specified configuration file is not "
                             "accessible: "
                          << command_line_parameters->configuration_file_name << std::endl)
  ...
  // ds reset and check for other command line parameters, potentially OVERWRITING the ones set in the configuration
  // file
  number_of_checked_parameters = 1;
  while (number_of_checked_parameters < argc_)
  {
    if (!std::strcmp(argv_[number_of_checked_parameters], "-topic-image-left") ||
        !std::strcmp(argv_[number_of_checked_parameters], "-il"))
    {
      number_of_checked_parameters++;
      if (number_of_checked_parameters == argc_)
      {
    ...
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-topic-image-right") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-ir"))
    {
      number_of_checked_parameters++;
      if (number_of_checked_parameters == argc_)
      {
    ...
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-topic-camera-info-left") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-cl"))
    {
      number_of_checked_parameters++;
      if (number_of_checked_parameters == argc_)
      {
    ...
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-topic-camera-info-right") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-cr"))
    {
      number_of_checked_parameters++;
      if (number_of_checked_parameters == argc_)
      {
    ...
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-h") ||
             !std::strcmp(argv_[number_of_checked_parameters], "--h"))
    {
      std::cerr << banner << std::endl;
      throw std::runtime_error("help requested");
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-help") ||
             !std::strcmp(argv_[number_of_checked_parameters], "--help"))
    {
      std::cerr << banner << std::endl;
      throw std::runtime_error("help requested");
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-use-gui") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-ug"))
    {
      command_line_parameters->option_use_gui = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-open-loop") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-ol"))
    {
      command_line_parameters->option_disable_relocalization = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-show-top") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-st"))
    {
      command_line_parameters->option_show_top_viewer = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-drop-framepoints") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-df"))
    {
      command_line_parameters->option_drop_framepoints = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-equalize-histogram") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-eh"))
    {
      command_line_parameters->option_equalize_histogram = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-depth-mode") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-dm"))
    {
      command_line_parameters->tracker_mode = CommandLineParameters::TrackerMode::RGB_DEPTH;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-use-odometry") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-uo"))
    {
      command_line_parameters->option_use_odometry = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-recover-landmarks") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-rl"))
    {
      command_line_parameters->option_recover_landmarks = true;
    }
    else if (!std::strcmp(argv_[number_of_checked_parameters], "-configuration") ||
             !std::strcmp(argv_[number_of_checked_parameters], "-c"))
    {
      number_of_checked_parameters++;
    }
    ...
    {
      if (command_line_parameters->dataset_file_name.length() == 0)
      {
        command_line_parameters->dataset_file_name = argv_[number_of_checked_parameters];
      }
    }
    number_of_checked_parameters++;
  }
  ...
  try
  {
    ...
    switch (command_line_parameters->tracker_mode)
    {
      case CommandLineParameters::TrackerMode::RGB_STEREO:
      {
        ...
        PARSE_PARAMETER(configuration, stereo_framepoint_generation, stereo_framepoint_generator_parameters,
                        maximum_matching_distance_triangulation, int32_t)
        PARSE_PARAMETER(configuration, stereo_framepoint_generation, stereo_framepoint_generator_parameters,
                        minimum_disparity_pixels, real)
        PARSE_PARAMETER(configuration, stereo_framepoint_generation, stereo_framepoint_generator_parameters,
                        maximum_epipolar_search_offset_pixels, int32_t)
        break;
      }
      case CommandLineParameters::TrackerMode::RGB_DEPTH:
      {
        ...
        // FramepointGeneration (SPECIFIC)
        PARSE_PARAMETER(configuration, depth_framepoint_generation, depth_framepoint_generator_parameters,
                        maximum_depth_near_meters, real)
        PARSE_PARAMETER(configuration, depth_framepoint_generation, depth_framepoint_generator_parameters,
                        maximum_depth_far_meters, real)
        break;
      }
      default:
      {
    ...
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters, descriptor_type,
                    std::string)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    target_number_of_keypoints_tolerance, real)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    detector_threshold_minimum, int32_t)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    detector_threshold_maximum, int32_t)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    detector_threshold_maximum_change, real)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    number_of_detectors_vertical, int32_t)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    number_of_detectors_horizontal, int32_t)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    matching_distance_tracking_threshold, int32_t)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    maximum_reliable_depth_meters, real)
    ...
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    minimum_projection_tracking_distance_pixels, int32_t)
    PARSE_PARAMETER(configuration, base_framepoint_generation, framepoint_generation_parameters,
                    maximum_projection_tracking_distance_pixels, int32_t)
    ...
    PARSE_PARAMETER(configuration, relocalization, relocalizer_parameters, preliminary_minimum_interspace_queries,
                    Count)
    ...
    PARSE_PARAMETER(configuration, graph_optimization, graph_optimizer_parameters,
                    number_of_frames_per_bundle_adjustment, Count)
    ...
    PARSE_PARAMETER(configuration, graph_optimization, graph_optimizer_parameters,
                    base_information_frame_factor_for_translation, real)
    PARSE_PARAMETER(configuration, graph_optimization, graph_optimizer_parameters, enable_robust_kernel_for_poses, bool)
    PARSE_PARAMETER(configuration, graph_optimization, graph_optimizer_parameters, enable_robust_kernel_for_landmarks,
                    bool)

    // ds done
    LOG_INFO(std::cerr << "ParameterCollection::parseFromFile|successfully loaded configuration from file: "
                       << filename_ << std::endl)
    LOG_INFO(std::cerr << "ParameterCollection::parseFromFile|number of imported parameters: "
                       << number_of_parameters_parsed << "/" << number_of_parameters_detected << std::endl)
  }
  catch (const YAML::BadFile& exception_)
  {
    LOG_ERROR(std::cerr << "ParameterCollection::parseFromFile|unable to parse configuration file: " << filename_
                        << " - exception: '" << exception_.what() << "'" << std::endl)
  }
}
...
void ParameterCollection::validateParameters()
{
  ...
  {
    LOG_ERROR(std::cerr << "ParameterCollection::validateParameters|empty value entered for parameter: "
                           "-topic-image-left (-il) (enter -h for help)"
                        << std::endl)
  ...
  {
    LOG_ERROR(std::cerr << "ParameterCollection::validateParameters|empty value entered for parameter: "
                           "-topic-image-right (-ir) (enter -h for help)"
                        << std::endl)
...
void ParameterCollection::setMode(const CommandLineParameters::TrackerMode& mode_)
{
  ...
  switch (mode_)
  {
    case CommandLineParameters::TrackerMode::RGB_STEREO:
    {
      if (!stereo_framepoint_generator_parameters)
      {
        stereo_framepoint_generator_parameters =
            new StereoFramePointGeneratorParameters(command_line_parameters->logging_level);
      }
    ...
    case CommandLineParameters::TrackerMode::RGB_DEPTH:
    {
      if (!depth_framepoint_generator_parameters)
      {
        depth_framepoint_generator_parameters =
            new DepthFramePointGeneratorParameters(command_line_parameters->logging_level);
      }
...
void ParameterCollection::print() const
{
  ...
  {
    command_line_parameters->print();
  }
  if (world_map_parameters)
  {
    world_map_parameters->print();
  }
  if (stereo_framepoint_generator_parameters)
  {
    stereo_framepoint_generator_parameters->print();
  }
  if (depth_framepoint_generator_parameters)
  {
    depth_framepoint_generator_parameters->print();
  }
  if (stereo_tracker_parameters)
  {
    stereo_tracker_parameters->print();
  }
  if (depth_tracker_parameters)
  {
    depth_tracker_parameters->print();
  }
  if (relocalizer_parameters)
  {
    relocalizer_parameters->print();
  }
  if (graph_optimizer_parameters)
  {
    graph_optimizer_parameters->print();
  }
}
}
