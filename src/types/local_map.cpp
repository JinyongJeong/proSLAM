...
LocalMap::LocalMap(FramePointerVector& frames_, const LocalMapParameters* parameters_, LocalMap* local_map_root_,
  ...
  , _parameters(parameters_)
{
  ...
  for (Frame* frame : frames_)
  {
    ...
    for (FramePoint* frame_point : frame->points())
    {
      ...
      {
        // ds create HBST matchables based on available landmark descriptors TODO move this operation into a method of
        // the landmark
  ...
  {
    LOG_INFO(std::cerr << "LocalMap::LocalMap|" << _identifier << "|pruning landmarks from: " << landmarks_added.size()
                       << " to: " << maximum_number_of_landmarks << std::endl)
  ...
  {
    LOG_WARNING(std::cerr << "LocalMap::LocalMap|creating local map with low landmark number: "
                          << landmarks_added.size() << std::endl)
  }
}
...
void LocalMap::replace(Landmark* landmark_old_, Landmark* landmark_new_)
{
  ...
  {
    LOG_WARNING(std::cerr << "LocalMap::replace|" << _identifier
                          << "|unable to erase old landmark with ID: " << landmark_old_->identifier() << std::endl)
  }
  ...
  else
  {
    ...
    _landmarks.insert(std::make_pair(landmark_new_->identifier(),
                                     LandmarkState(landmark_new_, _world_to_local_map * landmark_new_->coordinates())));
  }
}

void LocalMap::setLocalMapToWorld(const TransformMatrix3D& local_map_to_world_,
                                  const bool update_landmark_world_coordinates_)
{
