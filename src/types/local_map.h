...
namespace proslam
{
//! @class this class condenses a group of Frame objects into a single Local Map object, which used for relocalization
...
public:
  ...
  {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ClosureConstraint(const LocalMap* local_map_, const TransformMatrix3D& relation_,
                      const Closure::CorrespondencePointerVector& landmark_correspondences_, const real& omega_ = 1)
      : local_map(local_map_), relation(relation_), landmark_correspondences(landmark_correspondences_), omega(omega_)
    {
    }
  ...
  {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    LandmarkState(Landmark* landmark_, PointCoordinates coordinates_in_local_map_)
      : landmark(landmark_), coordinates_in_local_map(coordinates_in_local_map_)
    {
    }
  ...
  typedef std::map<const Identifier, LandmarkState, std::less<Identifier>,
                   Eigen::aligned_allocator<LandmarkStateMapElement>>
      LandmarkStateMap;
...
protected:
  ...
  LocalMap(FramePointerVector& frames_, const LocalMapParameters* parameters_, LocalMap* local_map_root_ = nullptr,
...
public:
  ...
  //! @param[in] transform_query_to_reference_ the spatial relation between query and reference (from query to
  //! reference)
  ...
  void addCorrespondence(const LocalMap* local_map_reference_, const TransformMatrix3D& query_to_reference_,
                         const Closure::CorrespondencePointerVector& landmark_correspondences_, const real& omega_ = 1)
  {
    _closures.push_back(
        ClosureConstraint(local_map_reference_, query_to_reference_, landmark_correspondences_, omega_));
  }
...
public:
  ...
  inline const TransformMatrix3D& localMapToWorld() const
  {
    return _local_map_to_world;
  }
  inline const TransformMatrix3D& worldToLocalMap() const
  {
    return _world_to_local_map;
  }
  void setLocalMapToWorld(const TransformMatrix3D& local_map_to_world_,
                          const bool update_landmark_world_coordinates_ = false);

  inline LocalMap* root()
  {
    return _root;
  }
  void setRoot(LocalMap* root_)
  {
    _root = root_;
  }
  inline LocalMap* previous()
  {
    return _previous;
  }
  void setPrevious(LocalMap* local_map_)
  {
    _previous = local_map_;
  }
  inline LocalMap* next()
  {
    return _next;
  }
  void setNext(LocalMap* local_map_)
  {
    _next = local_map_;
  }
  inline Frame* keyframe() const
  {
    return _keyframe;
  }
  inline const FramePointerVector& frames() const
  {
    return _frames;
  }
  inline LandmarkStateMap& landmarks()
  {
    return _landmarks;
  }
  inline AppearanceVector& appearances()
  {
    return _appearances;
  }
  inline const AppearanceVector& appearances() const
  {
    return _appearances;
  }

  // ds TODO purge this
  inline const ClosureConstraintVector& closures() const
  {
    return _closures;
  }
