...
namespace proslam
{
BaseFramePointGenerator::BaseFramePointGenerator(BaseFramePointGeneratorParameters* parameters_)
...
void BaseFramePointGenerator::configure()
{
  ...
  LOG_INFO(std::cerr << "BaseFramePointGenerator::configure|focal length (pixels): " << _focal_length_pixels
                     << std::endl)
...
// ds allocate descriptor extractor TODO enable further support and check BIT SIZES
  ...
  const real pixel_rows_per_detector =
      static_cast<real>(_number_of_rows_image) / _parameters->number_of_detectors_vertical;
  const real pixel_cols_per_detector =
      static_cast<real>(_number_of_cols_image) / _parameters->number_of_detectors_horizontal;
...
#endif
      _detector_regions[r][c] =
          cv::Rect(std::round(c * pixel_cols_per_detector), std::round(r * pixel_rows_per_detector),
                   pixel_cols_per_detector, pixel_rows_per_detector);
  ...
  _number_of_cols_bin =
      std::floor(static_cast<real>(_camera_left->numberOfImageCols()) / _parameters->bin_size_pixels) + 1;
  _number_of_rows_bin =
      std::floor(static_cast<real>(_camera_left->numberOfImageRows()) / _parameters->bin_size_pixels) + 1;

  // ds compute target number of points
  _target_number_of_keypoints = _number_of_cols_bin * _number_of_rows_bin;
  LOG_INFO(std::cerr << "BaseFramePointGenerator::configure|current target number of points: "
                     << _target_number_of_keypoints << std::endl)
  ...
  LOG_INFO(std::cerr << "BaseFramePointGenerator::configure|current target number of points per image region: "
                     << _target_number_of_keypoints_per_detector << std::endl)
  ...
  LOG_INFO(std::cerr << "BaseTracker::configure|number of horizontal bins: " << _number_of_cols_bin
                     << " size: " << _parameters->bin_size_pixels << std::endl)
  LOG_INFO(std::cerr << "BaseTracker::configure|number of vertical bins: " << _number_of_rows_bin
                     << " size: " << _parameters->bin_size_pixels << std::endl)
...
// ds current threshold for this detector
      ...
      const real delta = (static_cast<real>(keypoints_per_detector.size()) - _target_number_of_keypoints_per_detector) /
                         _target_number_of_keypoints_per_detector;
      ...
      // detector마다 thershold를 다르게 해서 일정수의 detection된 feature의 수를 유지한다.
      // 이게 detector를 여러개를 사용하는 이유! (영역마다 threshold를 조금씩 다르게)
      ...
      std::for_each(keypoints_per_detector.begin(), keypoints_per_detector.end(),
                    [&offset](cv::KeyPoint& keypoint_) { keypoint_.pt += offset; });

      // ds add to complete vector
      // 영역마다 일정수의 key point를 추출해서 더해나감!
...
void BaseFramePointGenerator::computeDescriptors(const cv::Mat& intensity_image_, std::vector<cv::KeyPoint>& keypoints_,
                                                 cv::Mat& descriptors_)
{
...
void BaseFramePointGenerator::track(Frame* frame_, Frame* frame_previous_,
