...
namespace proslam
{
StereoFramePointGenerator::StereoFramePointGenerator(StereoFramePointGeneratorParameters* parameters_)
  : BaseFramePointGenerator(parameters_), _parameters(parameters_)
{
...
void StereoFramePointGenerator::configure()
{
  ...
  LOG_INFO(std::cerr << "StereoFramePointGenerator::configure|number of epipolar lines considered for stereo matching: "
                     << _epipolar_search_offsets_pixel.size() << std::endl)
...
void StereoFramePointGenerator::initialize(Frame* frame_, const bool& extract_features_)
{
  ...
  if (extract_features_)
  {
    ...
    // 이미지에서 keypoint를 뽑아서 keypointLeft와 keypointright에 넣는다
    ...
    // 왼쪽/오른쪽 각 keypoint들의 descriptor 추출
    ...
    LOG_DEBUG(std::cerr << "StereoFramePointGenerator::initialize|extracted features L: "
                        << frame_->keypointsLeft().size() << " R: " << frame_->keypointsRight().size() << std::endl)
    ...
    else
    {
      ...
      const real ratio_available_points =
          std::min(static_cast<real>(_number_of_detected_keypoints) / _target_number_of_keypoints, 1.0);
      _current_maximum_descriptor_distance_triangulation =
          std::max(ratio_available_points * _parameters->maximum_matching_distance_triangulation,
                   0.1 * SRRG_PROSLAM_DESCRIPTOR_SIZE_BITS);
    }
  }
                                      ...
                                      const bool track_by_appearance_)
{
  ...
  for (FramePoint* point_previous : framepoints_previous)
  {
    ...
    if (feature_left)
    {
      ...
      const int32_t row_projection_right_corrected =
          point_in_image_right.y() / point_in_image_right.z() - projection_error.y;
      ...
      col_end_point =
          std::min(col_projection_right_corrected + _projection_tracking_distance_pixels + 1, feature_left->col);
      ...
      if (feature_right)
      {
        ...
        //        if (cv::norm(feature_right->descriptor, point_previous->descriptorRight(),
        //        SRRG_PROSLAM_DESCRIPTOR_NORM) > _parameters->matching_distance_tracking_threshold) {
        //          continue;
        //        }
...
const PointCoordinates StereoFramePointGenerator::getPointInLeftCamera(
    const cv::Point2f& image_coordinates_left_, const cv::Point2f& image_coordinates_right_) const
{
  ...
  position_in_left_camera.y() =
      1 / _f_y * ((image_coordinates_left_.y + image_coordinates_right_.y) / 2.0 - _c_y) * position_in_left_camera.z();
...
void StereoFramePointGenerator::compute(Frame* frame_)
{
  ...
  for (const int32_t& epipolar_offset : _epipolar_search_offsets_pixel)
  {
    ...
    for (uint32_t index_L = 0; index_L < features_left.size(); index_L++)
    {
      ...
      if (index_R == features_right.size())
      {
        break;
      }
      ...
      while (feature_left->row == features_right[index_search_R]->row + epipolar_offset)
      {
        ...
        const real descriptor_distance = cv::norm(feature_left->descriptor, features_right[index_search_R]->descriptor,
        ...
        index_search_R++;
        if (index_search_R == features_right.size())
        {
          break;
        }
      }
      ...
      if (descriptor_distance_best < _current_maximum_descriptor_distance_triangulation)
      {
        ...
        if (_parameters->enable_keypoint_binning)
        {
          ...
          {
            // ds if the point in the bin is not tracked, we prefer points with maximal disparity (= maximally accurate
            // depth estimate)
            if (!_bin_map_left[row_bin][col_bin]->previous() &&
                ...
                framepoint->descriptorDistanceTriangulation() <=
                    _bin_map_left[row_bin][col_bin]->descriptorDistanceTriangulation())
            {
    ...
    LOG_DEBUG(std::cerr << "StereoFramePointGenerator::compute|epipolar offset: " << epipolar_offset
                        << " number of unmatched features L: " << features_left.size()
                        << " R: " << features_right.size() << std::endl)
  }
  ...
  const real available_point_ratio =
      static_cast<real>(number_of_points_tracked + framepoints_new.size()) / _target_number_of_keypoints;
  ...
  else
  {
    ...
    if (_parameters->enable_keypoint_binning)
    {
      LOG_WARNING(std::cerr << "StereoFramePointGenerator::compute|skipped binning due to low point density: "
                            << available_point_ratio << std::endl)
      for (Index row = 0; row < _number_of_rows_bin; ++row)
      {
        for (Index col = 0; col < _number_of_cols_bin; ++col)
        {
  ...
  //      _epipolar_search_offsets_pixel.push_back(-_maximum_epipolar_search_offset_pixels);
  //      LOG_DEBUG(std::cerr << "StereoFramePointGenerator::compute|checking more epipolar lines in next input because
  //      of low triangulation ratio: " << triangulation_ratio << std::endl)
  //    }
  //  }
  //
