...
IntensityFeature* IntensityFeatureMatcher::getMatchingFeatureInRectangularRegion(
    const int32_t& row_reference_, const int32_t& col_reference_, const cv::Mat& descriptor_reference_,
    const int32_t& row_start_point, const int32_t& row_end_point, const int32_t& col_start_point,
    const int32_t& col_end_point, const real& maximum_descriptor_distance_tracking_, const bool track_by_appearance_,
  ...
  if (track_by_appearance_)
  {
    for (int32_t row = row_start_point; row < row_end_point; ++row)
    {
      for (int32_t col = col_start_point; col < col_end_point; ++col)
      {
        ...
        {
          const real descriptor_distance =
              cv::norm(descriptor_reference_, feature_lattice[row][col]->descriptor, SRRG_PROSLAM_DESCRIPTOR_NORM);
      ...
      for (int32_t col = col_start_point; col < col_end_point; ++col)
      {
        if (feature_lattice[row][col])
        {
          ...
          if (descriptor_distance < maximum_descriptor_distance_tracking_)
          {
            ...
            const uint32_t projection_distance_pixels =
                row_distance_pixels * row_distance_pixels + col_distance_pixels * col_distance_pixels;
