...
public:
  ...
  void process(const cv::Mat& intensity_image_left_, const cv::Mat& intensity_image_right_,
               const double& timestamp_image_left_seconds_ = 0, const bool& use_odometry_ = false,
  ...
  void writeTrajectoryKITTI(const std::string& file_name_ = "") const
  {
    if (_world_map)
    {
      _world_map->writeTrajectoryKITTI(file_name_);
    }
  }
  ...
  void writeTrajectoryTUM(const std::string& file_name_ = "") const
  {
    if (_world_map)
    {
      _world_map->writeTrajectoryTUM(file_name_);
    }
  }
  ...
  void writeTrajectory(
      std::vector<Eigen::Matrix<RealType, 4, 4>, Eigen::aligned_allocator<Eigen::Matrix<RealType, 4, 4>>>& poses_) const
  {
    if (_world_map)
    {
      _world_map->writeTrajectory<RealType>(poses_);
    }
  }
  ...
  template <typename RealType>
  void writeTrajectoryWithTimestamps(
      std::vector<std::pair<RealType, Eigen::Transform<RealType, 3, Eigen::Isometry>>>& poses_) const
  {
    if (_world_map)
    {
      _world_map->writeTrajectoryWithTimestamps<RealType>(poses_);
    }
  }
...
public:
  ...
  const bool isViewerOpen() const
  {
    return _is_viewer_open;
  }
  const double currentFPS() const
  {
    return _current_fps;
  }
  const double averageNumberOfLandmarksPerFrame() const
  {
    return _tracker->totalNumberOfLandmarks() / _number_of_processed_frames;
  }
  const double averageNumberOfTracksPerFrame() const
  {
    return _tracker->totalNumberOfTrackedPoints() / _number_of_processed_frames;
  }
  const Count numberOfRecursiveRegistrations() const
  {
    return _tracker->numberOfRecursiveRegistrations();
  }
  const real meanTrackingRatio() const
  {
    return _tracker->meanTrackingRatio();
  }
  const real meanTriangulationRatio() const
  {
    return dynamic_cast<StereoFramePointGenerator*>(_tracker->framepointGenerator())->meanTriangulationSuccessRatio();
  }
